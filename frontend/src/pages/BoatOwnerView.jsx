...
export default function BoatOwnerView() {
  const { id } = useParams();
    ...
    <div className="max-w-5xl mx-auto mt-10 p-6 bg-white rounded-xl shadow-md border space-y-10">
      <div className="border-b pb-4">
        <h1 className="text-3xl text-center font-bold text-indigo-700 mb-1">
          👨‍🌾 Boat Owner Details - {owner.name}
        </h1>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        <Field label="Name" value={owner.name} />
        <Field label="Mobile No" value={owner.number} />
        <Field label="Email" value={owner.email} />
        <Field label="Aadhar No" value={owner.adhar_no} />
        <Field label="Date of Birth" value={owner.dob} />
        <Field label="Boats Owned" value={owner.boat_owned} />
        <Field label="District" value={owner.district?.district_name} />
      ...
      <div className="pt-6 border-t">
        ...
        {owner.boat_family_members.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left text-gray-700 border">
              ...
              <tbody>
                {owner.boat_family_members.map((member) => (
