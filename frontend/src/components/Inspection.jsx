...
const checklistItems = [
  "Hull Integrity","Engine Performance","Life Jackets","Fire Extinguisher","First Aid Kit",
  "Communication Radio","Gps System","Navigation Lights","Fuel Lines","Electrical Wiring",
  "Propeller","Steering",
];
...
const Textarea = ({ label, value, onChange, placeholder }) => (
  <div className="mb-6">
    <label className="block text-sm font-medium mb-2">{label}</label>
    <textarea rows={3} className="border w-full p-2 rounded focus:ring-2 focus:ring-violet-500"
...
/* ================================= Records ================================= */
const sampleRecords = [
  {reg:"UP-VAR-001",date:"2024‑01‑15",inspector:"Rajesh Kumar",status:"Passed",color:"green",valid:"2025‑01‑15",district:"Varanasi"},
  {reg:"UP-LKO-005",date:"2024‑01‑10",inspector:"Sunil Singh",status:"Failed",color:"red",valid:"N/A",district:"Lucknow"},
];

const Records = () => (
  <div className="mt-8">
    <h2 className="text-2xl font-semibold text-center mb-1">Inspection Records</h2>
    <p className="text-gray-600 text-center mb-6">History of boat inspections (static demo)</p>

    <div className="overflow-x-auto rounded-lg shadow bg-white">
      <table className="min-w-full text-sm">
        <thead className="bg-gray-100">
          <tr>{["Boat Registration","Inspection Date","Inspector","Status","Valid Until","District","Actions"]
            .map((h)=> <th key={h} className="px-4 py-3 font-medium text-gray-700 border-b">{h}</th>)}
          </tr>
        </thead>
        <tbody>
          {sampleRecords.map((r)=>(
            <tr key={r.reg} className="hover:bg-gray-50">
              <td className="px-4 py-3 border-b">{r.reg}</td>
              <td className="px-4 py-3 border-b">{r.date}</td>
              <td className="px-4 py-3 border-b">{r.inspector}</td>
              <td className="px-4 py-3 border-b">
                <span className={`bg-${r.color}-100 text-${r.color}-600 px-3 py-1 rounded-full text-xs font-medium`}>
                  {r.status}</span>
              </td>
              <td className="px-4 py-3 border-b">{r.valid}</td>
              <td className="px-4 py-3 border-b">{r.district}</td>
              <td className="px-4 py-3 border-b">
                <button className="text-sm px-4 py-1 border rounded hover:bg-gray-100">View</button>
              </td>
            </tr>
          ))}
        </tbody>
...
const Schedule = () => (
  <div className="grid md:grid-cols-2 gap-6">
    ...
    <div className="bg-white p-6 rounded-lg shadow">
      ...
      {upcoming.map((u)=>(
        <div key={u.reg} className="flex justify-between bg-yellow-50 p-3 rounded mb-3">
          <div><div className="font-medium">{u.reg}</div>
              <div className="text-sm text-gray-600">{u.district}</div></div>
          <div className="text-sm text-right">
            <div className="text-gray-700">Due: {u.due}</div>
            <span className="bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full">{u.days} days</span>
          </div>
        </div>
      ))}
    </div>
    ...
    <div className="bg-white p-6 rounded-lg shadow">
      ...
      <div className="grid grid-cols-7 gap-2 text-sm text-gray-700 mb-4">
        ...
        {Array.from({length:35}).map((_,i)=>(
          <div key={i} className="h-10 flex items-center justify-center border border-gray-200 rounded">
            {i>2? i-2: ""}
          </div>
        ))}
      </div>
      <div className="flex space-x-6 text-sm">
        {[
          ...
          <div key={l} className="flex items-center space-x-2">
            <div className={`w-3 h-3 ${c} rounded-full`} /><span>{l}</span>
          </div>
        ))}
      </div>
    </div>
...
const Analytics = () => {
  const kpis = [
    {label:"Total Boats",value:1247,sub:"Registered fleet",color:"bg-blue-500"},
    {label:"Inspected",value:856,sub:"This year",color:"bg-green-500"},
    {label:"Pass Rate",value:"87%",sub:"Success rate",color:"bg-violet-500"},
    {label:"Pending",value:391,sub:"Awaiting inspection",color:"bg-orange-500"},
  ];
  const passed=742,failed=67,conditional=47,total=1247,inspected=passed+failed+conditional;
  ...
  return (
    <div className="space-y-8 mb-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {kpis.map((k)=>(
          <div key={k.label} className="bg-white p-6 rounded-lg shadow relative">
            <span className={`absolute top-4 right-4 w-3 h-3 rounded-full ${k.color}`} />
      ...
      {/* progress */}
      <div className="bg-white p-6 rounded-lg shadow">
        <h2 className="text-xl font-semibold mb-1">Inspection Status Overview</h2>
        <p className="text-sm text-gray-600 mb-4">Year‑to‑date progress</p>
        <div className="relative w-full h-3 bg-gray-200 rounded mb-2">
          <div className="absolute top-0 left-0 h-3 bg-gray-900 rounded" style={{width:pct}} />
          <span className="absolute right-0 -top-5 text-sm text-gray-600">{inspected}/{total}</span>
        </div>
